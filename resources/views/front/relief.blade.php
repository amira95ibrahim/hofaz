        ...
        <section>
            <div class="container pt-70 pb-40">
                <div class="section-content">
                    <div class="row">
                        <div class="col-md-12">
                            <h2>{{ $reliefsPageDetails->title }}</h2>
                            <p>{{ $reliefsPageDetails->details }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section>
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="wrapper">
                            <form class="form-group" action="{{ route('relief') }}" method="get" id="filterForm">
                                <div class="filter-tab">
                                    <div class="country-gender">
                                        <select class="form-control filterReliefs" name="country">
                                            <option selected="" value="">@lang('kafala.select_country')</option>
                                            @foreach($reliefsCountries as $country)
                                ...
                                <div id="flip-card{{ $key }}">
                                    <div class="flip-card-front{{ $key }}">
                                        <div class="causes bg-white maxwidth500 mb-30">
                                            <div class="thumb">
                                                <a href="{{ route('relief.show', $relief->id) }}">
                                                    <img src="{{ asset($relief->image) }}" alt=""
                                                         class="img-fullwidth">
                                                </a>
                                            </div>
                                            <div class="causes-details border-1px bg-white clearfix p-20 pt-10 pb-20">
                                                <h4 class="font-20 text-uppercase"><a href="{{ route('relief.show', $relief->id) }}">{{ $relief->name }}</a></h4>
                                                @if($relief->show_remaining)
                                                    <ul class="list-inline font-weight-600 font-16 clearfix mt-15">
                                                        <li class="pull-left font-weight-400 text-black-333 pr-0">@lang('relief.raised')
                                                            :
                                                            <span class="text-theme-colored font-weight-700">{{ number_format($relief->cost - $relief->paid) }} @lang('relief.KWD')</span>
                                                        </li>
                                                        <li class="pull-right font-weight-400 text-black-333 pr-0">@lang('relief.goal')
                                                            :
                                                            <span class="text-theme-colored font-weight-700">{{ number_format($relief->cost) }} @lang('relief.KWD')</span>
                                                        </li>
                                                    </ul>
                                                    <div class="progress-item mt-15">
                                                        <div class="progress mb-0">
                                                            <div data-percent="{{ round(($relief->paid * 100) / $relief->cost) }}" class="progress-bar"><span
                                                                    class="percent">0</span></div>
                                                        </div>
                                                    </div>
                                                @else
                                                    <div class="progress-item mt-30 mb-25">
                                                        @lang('relief.share_price') :
                                                        <span class="text-theme-colored font-weight-700">{{ $relief->cost }} @lang('relief.KWD')</span>
                                                    </div>
                                                @endif
{{--                                                <p class="mt-15">{{ $relief->description }}</p>--}}
                                            </div>
                                        </div>
                                            ...
                                            <div class="causes-details border-1px bg-white clearfix p-20 pt-10 pb-20">
                                                ...
                                                <div class="donation-amount">
                                                    ...
                                                    <div class="slect-amount">
                                                        <div class="button">
                                                            <input type="radio" id="a25" name="check-substitution-2"
                                                                   onclick="updateAmount('{{ $relief->id }}', '{{ $relief->initial_amount }}')"/>
                                                            <label class="btn btn-default"
                                                                   for="a25">{{ $relief->initial_amount }} @lang('waqf.KWD')</label>
                                                        </div>
                                                        <div class="button">
                                                            <input type="radio" id="a50" name="check-substitution-2"
                                                                   onclick="updateAmount('{{ $relief->id }}', '{{ ($relief->show_remaining) ? $relief->initial_amount + 10 : $relief->initial_amount * 2 }}')"/>
                                                            <label class="btn btn-default"
                                                                   for="a50">{{ ($relief->show_remaining) ? $relief->initial_amount + 10 : $relief->initial_amount * 2 }} @lang('waqf.KWD')</label>
                                                        </div>
                                                        <div class="button">
                                                            <input type="radio" id="a75" name="check-substitution-2"
                                                                   onclick="updateAmount('{{ $relief->id }}', '{{ ($relief->show_remaining) ? $relief->initial_amount + 20 : $relief->initial_amount * 3 }}')"/>
                                                            <label class="btn btn-default"
                                                                   for="a75">{{ ($relief->show_remaining) ? $relief->initial_amount + 20 : $relief->initial_amount * 3 }} @lang('waqf.KWD')</label>
                                                        </div>
                                                        <div class="button">
                                                            <input type="radio" id="a75" name="check-substitution-2"
                                                                   onclick="updateAmount('{{ $relief->id }}', '{{ ($relief->show_remaining) ? $relief->initial_amount + 30 : $relief->initial_amount * 4 }}')"/>
                                                            <label class="btn btn-default"
                                                                   for="a75">{{ ($relief->show_remaining) ? $relief->initial_amount + 30 : $relief->initial_amount * 4 }} @lang('waqf.KWD')</label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <form class="mt-1">
                                                        <div class="input-group">
                                                            <input type="number" name="amount"
                                                                   ...
                                                                   id="{{ 'product_relief_' . $relief->id . '_amount' }}">
                                                            <input type="hidden" id="{{ 'product_relief_' . $relief->id . '_image' }}" value="{{ $relief->image }}">
                                                            <input type="hidden" id="{{ 'product_relief_' . $relief->id . '_name' }}" value="{{ $relief->name }}">
                                                    ...
                                                    <a href="javascript:void(0);" 
                                                        onclick="donateNow('{{ 'relief_'  . $relief->id }}', 'relief', {{$relief->id}}, getUserIdFromURL())"
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.filterReliefs').on('change', function(){
            $('#filterForm').submit();
        });

        function updateAmount(id, amount){
            $('#product_relief_' + id + '_amount').val(amount);
        }
    </script>
@endpush
